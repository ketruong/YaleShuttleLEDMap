...
class RealTimeVisualization(Visualization):
    def __init__(self):
        ...
        api_routes = transloc.Routes(transloc.Stations())
...
class MapState(object):
    def __init__(self):
	#sets all LEDs to 0 -> 5 lists of 0's 
        self.strips = {}
        for name, (index, length) in STRIPS.iteritems():
            self.strips[name] = [0] * length
    #changes the state of map	
    def diff_with(self, other):
        writes = []
        for k in STRIPS:
            this = self.strips[k]
	    #looks for changes in strips if other is not null
            if other:
                that = other.strips[k]
            else:
                that = None
...
class StripWrite(object):
    ...
    #returns string form of the Strip
    def __str__(self):
        return str((self.index, self.start, self.end, self.color))

    #returns if one strip is equal to another strip 	
